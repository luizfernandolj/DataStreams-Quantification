...
def run(dataset, window_size):
    ...
    exp = Experiment(train, test, window_size, clf, wrs, 'baseline')
    vet_accs, drift_points = exp.run_stream()
    vet_accs_table = pd.concat([vet_accs_table, vet_accs], axis=1)
    ...
    drift_table = pd.DataFrame({"IKS": drift_points_iks, "IBDD":drift_points_ibdd, "baseline":drift_points_wrs, "REAL":contexts})
