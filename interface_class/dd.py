...
class DriftDetector(ABC):
  ...
  def apply_quantification(self, new_instance : object, vet_accs : dict) -> dict:
    ...
    app.check_train(self.train.iloc[:, :-1], self.train.iloc[:, -1])
    proportions = app.aplly_qtf() # getting the proportions of each quantifier
    
    pos_scores = self.model.predict_proba(window)[:,1].tolist() # predicting the 'probabilities' of the window
    ...
    for qtf, proportion in proportions.items():
        ...
        thr = app.get_best_threshold(round(proportion, 3), pos_scores) # getting the threshold using the positive proportion
        print(f"qtf{name} - proportion{round(proportion, 3)} - threshold{thr}")
        #pdb.set_trace()
